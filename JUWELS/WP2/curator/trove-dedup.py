...
from datatrove.pipeline.writers.jsonl import ParquetWriter


# you can also change ngrams or the number of buckets and their size here
minhash_config = MinhashConfig(use_64bit_hashes=True)  # better precision -> fewer false positives (collisions)

ROOT = "/p/fastdata/trustllmd/WP2/trove"
BASE_PATH = f"{ROOT}/data/minhash/"
LOGS_FOLDER = f"{ROOT}/logs/minhash/"
LOCAL_LOGS_FOLDER = "minhash_logs"

TOTAL_TASKS = 1000

# this is the original data that we want to deduplicate
INPUT_READER = ParquetReader(f"{ROOT}/data/collected/", recursive=True)
